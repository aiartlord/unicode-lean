...
|---|---|---|---|
| D1 bidi rendering divergence | `bidiControlBalance`, `sourceDisplayDivergence`, `rtlInjection` | The Trojan Source property is closed against the bidi algorithm itself, for every input. Soundness: a line `safeForCodeContext` admits, carrying no right-to-left character, displays every retained position in logical order — `Unicode.TrojanSource.safeForCodeContext_displayOrder`, discharged through `Unicode.Bidi.Algorithm.reorderedInputIndices_ltr` and `Unicode.Bidi.Algorithm.reorderLine_ltr`, which walk every UAX #9 rule (X, W, N, I, L1, L2) and show no position moves. Completeness on such a line: a display/logical divergence is rejected — `Unicode.TrojanSource.displayDivergence_rejected`. On any line at all, a divergence comes from a bidi format control, which the scanner reports, or from a right-to-left character, which reorders by design — `Unicode.TrojanSource.displayDivergence_source`; the class table is tied to the control set by `Unicode.Bidi.Algorithm.formatting_class_range`. Every bidi control imbalance is reported, for every input: `Unicode.Security.Covert.BidiControlBalance.runWalk_depthAccounted`, `runWalk_stackConsistent`, `Unicode.TrojanSource.balanced_of_no_bidi_control`, `Unicode.TrojanSource.safeForCodeContext_balanced`. Which controls are reported is decided by purpose, not presence (`Unicode.Security.Display.BidiControlPurpose`): a span is purposeful only when its direct content is exactly its own direction and carries no ASCII code syntax — a right-to-left span around right-to-left text, a left-to-right span in right-to-left context around left-to-right text — and everything else fires wherever it sits, code, literal or comment: every unbalanced control, every balanced span with nothing of its own direction inside (`purposeless_balanced_empty_isolate`, `purposeless_lro_around_latin`, `purposeless_lri_around_latin`), every span mixing directions (`purposeless_rle_planted_letter`) and every span swallowing a quote or a bracket (`purposeless_rle_with_syntax`). A balanced embedding around a pure Arabic literal does not fire (`purposeful_rle_around_arabic`), nor does a Latin acronym isolated inside it (`purposeful_nested_acronym`). This is a property of the control span, never of the source region. The algorithm itself passes `BidiTest.txt` and `BidiCharacterTest.txt` in full. | theorem + conformance. The scope of the closed theorem is stated exactly: it covers lines without right-to-left characters, because a Hebrew or Arabic line reorders legitimately and no scanner should call that an attack. On a line that does carry right-to-left text, what the theorems give is the source of any divergence (a control or that text), and the purposeless-but-balanced case is carried by the `BidiControlPurpose` rule and the `sourceDisplayDivergence` vectors. What remains outside the rule is a pure right-to-left span placed so that its own text lands somewhere misleading; it moves no code and no syntax, and the text is visible to the reviewer. |
