/-
  ...
  with `ByteArray`. This module bridges the two by providing:
    ...
    * `decodeToCodepoints`  ByteArray → List Nat
    ...
    * `encodeCodepoints`    List Nat → ByteArray
...
def decodeToCodepoints (bs : ByteArray) : List Nat :=
...
def encodeCodepoint (cp : Nat) : ByteArray :=
  if cp < 0x80 then
    List.toByteArray [UInt8.ofNat cp]
...
def encodeCodepoints (cps : List Nat) : ByteArray :=
...
theorem encode_A : encodeCodepoint 0x0041 = List.toByteArray [0x41] := by decide

/-- LATIN CAPITAL LETTER A WITH GRAVE (`À`, U+00C0) encodes to two bytes:
    0xC3 0x80. -/
theorem encode_A_grave :
    encodeCodepoint 0x00C0 = List.toByteArray [0xC3, 0x80] := by decide

/-- `é` (U+00E9) encodes to 0xC3 0xA9. -/
theorem encode_eacute :
    encodeCodepoint 0x00E9 = List.toByteArray [0xC3, 0xA9] := by decide

/-- HIRAGANA LETTER A (`あ`, U+3042) encodes to three bytes:
    0xE3 0x81 0x82. -/
theorem encode_hiragana_a :
    encodeCodepoint 0x3042 = List.toByteArray [0xE3, 0x81, 0x82] := by decide

/-- GRINNING FACE (`😀`, U+1F600) encodes to four bytes:
    0xF0 0x9F 0x98 0x80. -/
theorem encode_emoji :
    encodeCodepoint 0x1F600 = List.toByteArray [0xF0, 0x9F, 0x98, 0x80] := by decide

/-- Decoding "Hi" (pure ASCII) yields the codepoint array. -/
theorem decode_ascii :
    decodeToCodepoints (List.toByteArray [0x48, 0x69]) = [0x48, 0x69] := by decide

/-- Decoding the UTF-8 bytes of `À` yields `[0x00C0]`. -/
theorem decode_A_grave :
    decodeToCodepoints (List.toByteArray [0xC3, 0x80]) = [0x00C0] := by decide
...
theorem toNFCBytes_ascii :
    toNFCBytes (List.toByteArray [0x48, 0x69]) = some (List.toByteArray [0x48, 0x69]) := by
  rewrite [toNFCBytes.eq_def,
    show decodeToCodepoints (List.toByteArray [0x48, 0x69]) = [0x0048, 0x0069]
      from by decide,
    NFC.toNFC_ascii]
  decide

theorem isNFCBytes_ascii :
    isNFCBytes (List.toByteArray [0x48, 0x69]) = true := by
...
theorem toNFCBytes_composes :
    toNFCBytes (List.toByteArray [0x41, 0xCC, 0x80])
      = some (List.toByteArray [0xC3, 0x80]) := by
  rewrite [toNFCBytes.eq_def,
    show decodeToCodepoints (List.toByteArray [0x41, 0xCC, 0x80])
...
theorem isNFCBytes_A_grave :
    isNFCBytes (List.toByteArray [0xC3, 0x80]) = true := by
  rewrite [isNFCBytes.eq_def, toNFCBytes.eq_def,
    show decodeToCodepoints (List.toByteArray [0xC3, 0x80]) = [0x00C0]
      from by decide,
...
theorem isNFCBytes_decomposed :
    isNFCBytes (List.toByteArray [0x41, 0xCC, 0x80]) = false := by
  rewrite [isNFCBytes.eq_def, toNFCBytes_composes]
  decide

/-- Invalid UTF-8 (lone continuation byte) returns none. -/
theorem toNFCBytes_invalid :
    toNFCBytes (List.toByteArray [0x80]) = none := by decide

theorem isNFCBytes_invalid :
    isNFCBytes (List.toByteArray [0x80]) = false := by decide
