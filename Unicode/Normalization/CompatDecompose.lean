...
def compatDecomposition (cp : Nat) : List Nat :=
...
def fullCompatDecomposeFuel : Nat → Nat → List Nat
  | 0,        cp => [cp]
  | fuel + 1, cp =>
...
/-- Fully compatibility-decompose a single codepoint. Thin wrapper that
    fixes `fuel = maxDepth`. -/
def fullCompatDecompose (cp : Nat) : List Nat :=
...
def compatDecomposeSequence (cps : List Nat) : List Nat :=
  cps.flatMap fullCompatDecompose
