...
def firstNonTransparentDown : Nat → List Nat → Nat → Option JoiningType
  | 0,        label, j  =>
    Function.const Nat (Function.const (List Nat) (none : Option JoiningType) label) j
  | fuel+1,   label, j  =>
    match label[j]? with
    | none    => none
    | some cp =>
...
def firstNonTransparentUp : Nat → List Nat → Nat → Option JoiningType
  | 0,        label, j  =>
    Function.const Nat (Function.const (List Nat) (none : Option JoiningType) label) j
  | fuel+1,   label, j  =>
    match label[j]? with
...
def joiningTypeBefore (label : List Nat) (i : Nat) : Option JoiningType :=
  if i = 0 then none
  else firstNonTransparentDown i label (i - 1)

/-- First non-Transparent joining type strictly after index `i`. -/
def joiningTypeAfter (label : List Nat) (i : Nat) : Option JoiningType :=
  firstNonTransparentUp (label.length - i) label (i + 1)
...
def checkContextJZwnj (label : List Nat) (i : Nat) : Bool :=
  let viramaPrev :=
    if i = 0 then false
...
def checkContextJZwj (label : List Nat) (i : Nat) : Bool :=
  if i = 0 then false
  else match label[i - 1]? with
    | none      => false
    | some prev => isVirama prev
...
/-- Check every ZWNJ / ZWJ occurrence from index `i` onward, `AND`-ing
    ...
    reduces in the kernel; callers pass `label.length`. Equivalent to a
    short-circuiting scan: the result is `false` iff some joiner in the
    label has an invalid context. -/
def checkJoinersFrom (label : List Nat) (fuel : Nat) (i : Nat) : Bool :=
  match fuel with
  | 0 => true
  | fuel + 1 =>
