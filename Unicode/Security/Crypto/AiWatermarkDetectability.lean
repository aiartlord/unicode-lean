...
/-- First start-position at which `pattern` appears as a
    ...
    patterns. -/
def containsSublist (pattern input : List Nat) : Option Nat :=
...
/-- The "AI-favored" lexical-pattern catalog parsed from the
    ...
    `statisticalTokenChoice` probe via `containsSublist`. -/
def aiFavoredVocabularyParsed : List (List Nat) :=
...
def aiFavoredVocabulary : List (List Nat) := [
  [100, 101, 108, 118, 101],
...
def detectWithContext (ctx : Context) (input : List Nat) : Verdict :=
  ...
  let vocabHit := aiFavoredVocabulary.findSome?
    (fun pattern => containsSublist pattern input)
