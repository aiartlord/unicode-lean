...
/-- Sub-threat enumeration for HomoglyphConfusable.
    ...
    Priority order (highest first):
      ...
      8. `intentionalConfusable` input clears every rung above, but folding
                            each codepoint to the representative of its UTS #39
                            intentional-confusable class (`intentional.txt`)
                            makes it match a curated target it did not match raw.
                            This catches the capital cross-script look-alikes the
                            case-folding skeleton keeps apart — a Greek capital
                            NU for a Latin N — where the lowercase forms the
                            skeleton folds to diverge.  Last, so it only speaks
                            for a look-alike no earlier rung named.
-/
inductive SubThreat where
  ...
  | intentionalConfusable (target : String)
...
/-- The input with every codepoint folded to the representative of its UTS #39
    intentional-confusable class.  `canonicalRep` is the identity outside the
    classes and on all ASCII, so this rewrites only the exotic look-alikes the
    standard names, leaving ordinary identifiers untouched. -/
def intentionalCanonicalInput (input : List Nat) : List Nat :=
  input.map Unicode.Generated.IntentionalConfusables.canonicalRep

/-- Positions carrying a UTS #39 intentional-confusable look-alike — the
    codepoints `canonicalRep` rewrites. -/
def intentionalConfusablePositions (input : List Nat) : List Nat :=
  input.zipIdx.filterMap (fun cpWithIdx =>
    if Unicode.Generated.IntentionalConfusables.canonicalRep cpWithIdx.1 != cpWithIdx.1
    then some cpWithIdx.2 else none)

/-- A curated target the input matches only after intentional-confusable
    canonicalization.  `none` when the input carries no intentional look-alike
    (canonicalization is a no-op) or the canonicalized form still matches no
    target.  Compares the canonicalized letter skeleton to each target's pinned
    letter skeleton, without `findTargetMatch`'s case-variant exclusion: a code
    point the standard names an intentional confusable is a substitution, never a
    mere case variant of the target, so Greek-capital `ΝΕΧΤ` is a hazard against
    `next` rather than the name `NEXT` spelled in another case.  This recovers
    the capital cross-script look-alikes whose case-folded skeletons diverge and
    the raw skeleton therefore misses. -/
def findIntentionalMatch (input : List Nat) : Option CanonicalTarget :=
  let cInput := intentionalCanonicalInput input
  if cInput = input then none
  else
    let cLetters := Unicode.Confusables.letterSkeleton cInput
    ((canonicalTargets.zip canonicalTargetSkeletons).find?
      (fun ts => decide (ts.snd = cLetters))).map (fun ts => ts.fst)
      ...
      if mc > 0 then
        ...
        if fwc > 0 then
          ...
          if !ctx.runningText && Nat.ble 2 sc && !Unicode.Restriction.isHighlyRestrictive input then
            ...
            match findIntentionalMatch input with
            | some t =>
              .hazard (.intentionalConfusable t.name) (intentionalConfusablePositions input) []
            | none   => .clear
  { input := input,
    classify := classification,
    skeleton := skel,
...
def SubThreat.tag : SubThreat → String
  ...
  | .intentionalConfusable target            =>
      Function.const String "IntentionalConfusable" target
