...
/-- Project an `F3Classification` to `(ClassificationKind, sub-threat-tag)`. -/
def projectClassify
    (c : F3Classification) : ClassificationKind × Option String :=
  if c.isClear then (.clear, none) else (.hazard, c.tag)

/-- Project an `F3Classification` to the positions array. -/
def projectPositions (c : F3Classification) : Array Nat :=
  c.positions
...
theorem covers_turkish :
    (rows.filter (·.sectionName = "TurkishCaseDivergence")).size ≥ 3 := by
  native_decide

theorem covers_lithuanian :
    (rows.filter (·.sectionName = "LithuanianCaseDivergence")).size ≥ 2 := by
  native_decide
