...
/-- Project a `C2Classification` to its `ClassificationKind` and
    sub-threat name.  The sub-threat tag matches the strings emitted
    in the fixture's column 2 (`Hazard:<SubThreatName>`). -/
def projectClassify
  ...
  | .hazard sub _ _ =>
    let tag : String := match sub with
      | .directPayload    _ _    => "DirectPayload"
      | .illegalTarget    _ _    => "IllegalTarget"
      | .embeddedAfterReg _ _    => "EmbeddedAfterRegistered"
      | .repeatedBase     _ _ _  => "RepeatedBase"
    (.hazard, some tag)
