...
-- The spot-check theorems evaluate `resolveAt` through the 39749-row DUCET
-- table; kernel reduction of those lookups exceeds the default recursion limit.
...
-- ═══════════════════════════════════════════════════════════════════════════════
-- §1 KERNEL-REDUCIBLE LOOKUP MIRROR
-- ═══════════════════════════════════════════════════════════════════════════════

/-- Longest DUCET entry whose key prefixes `cps`, found by a structural scan of
    the pinned `ducetEntriesList` (filter on the first codepoint, then fold for
    the longest key). The `resolveAt` runtime path uses a `Std.HashMap` index
    and `Id.run`/`for`, neither of which the kernel reduces; this mirror does
    the same lookup over the `List` (`List.isPrefixOf` for the key match) so
    `decide +kernel` can evaluate it. -/
def longestMatchAtList (cps : List Nat) : Option (DucetEntry × Nat) :=
  match cps.head? with
  | none    => none
  | some cp =>
