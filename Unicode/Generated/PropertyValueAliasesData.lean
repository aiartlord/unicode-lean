/-
  Unicode.Generated.PropertyValueAliasesData

  Materialized PropertyValueAliases.txt rows (UCD 17.0.0) as a `List` literal
  so value-name lookups reduce in the kernel. `PropertyValueRow` is defined
  here to avoid a circular import; the parser and drift gate live in
...
/-- One parsed PropertyValueAliases row: property short name, value short
    name, value long name, and any additional value aliases. -/
...
/-- Materialized property-value-alias rows in source order. -/
def parsedRowsList : List PropertyValueRow := [
  ⟨"AHex", "N", "No", #["F", "False"]⟩,
  ⟨"AHex", "Y", "Yes", #["T", "True"]⟩,
  ⟨"age", "1.1", "V1_1", #[]⟩,
  ⟨"age", "2.0", "V2_0", #[]⟩,
