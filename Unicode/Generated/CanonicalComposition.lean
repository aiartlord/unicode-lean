/-
  Unicode.Generated.CanonicalComposition

  The reverse of the canonical-decomposition mapping: for every
  non-excluded codepoint `P` whose canonical decomposition is exactly
  a two-element starter+combiner sequence `[d, c]`, the triple
  `(d, c, P)` records that `(d, c)` primary-composes to `P`.

  This is the lookup `Compose.primaryComposite?` consults for the
...
namespace Unicode.Generated.CanonicalComposition

/-- `(starter, combiner, composite)` for every non-excluded canonical
    two-element composition. See the module docstring. -/
def compositionPairs : List (Nat × Nat × Nat) :=
  [
...
-- ═══════════════════════════════════════════════════════════════════════════════
-- INTEGRITY GATE — `compositionPairs` must equal the canonical two-element
-- compositions derived from UnicodeData.txt (column 5, no tag) minus the
-- Full_Composition_Exclusion set, sorted by composite code point.
-- ═══════════════════════════════════════════════════════════════════════════════

/-- Raw text of `UnicodeData.txt`, embedded at compile time. -/
def unicodeDataRawCC : String := include_str "../Ucd/UnicodeData.txt"

def ccHexVal (c : Char) : Nat :=
  let n := c.toNat
  if n ≥ 0x30 ∧ n ≤ 0x39 then n - 0x30
  else if n ≥ 0x61 ∧ n ≤ 0x66 then n - 0x61 + 10
  else if n ≥ 0x41 ∧ n ≤ 0x46 then n - 0x41 + 10
  else 0
def ccHex (s : String) : Nat := s.foldl (fun acc c => acc * 16 + ccHexVal c) 0
def ccTrim (s : String) : String := (String.trimAscii s).toString
...
def ccDerive : Array (Nat × Nat × Nat) := Id.run do
  ...
  for line in unicodeDataRawCC.splitOn "\n" do
    ...
    let parts := (d.splitOn " ").filterMap (fun t =>
      let x := ccTrim t; if x.isEmpty then none else some (ccHex x))
    match parts with
    | [a, b] =>
...
#eval do
  unless compositionPairs == compositionPairsParsed do
    throw (IO.userError "CanonicalComposition drift: compositionPairs ≠ derived from UnicodeData.txt")

end Unicode.Generated.CanonicalComposition
