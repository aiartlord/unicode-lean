...
theorem lookup_target_non_source (source : Nat) (target : Array Nat) (cp : Nat)
...
def wParseRow (line : String) : Option (Nat × Array Nat) :=
  ...
  if !(decomp.startsWith "<wide>" || decomp.startsWith "<narrow>") then none
    ...
    let mapping := ((rest.splitOn " ").filterMap (fun tok =>
      let t := wTrim tok
      if t.isEmpty then none else some (wHex t))).toArray
    some (wHex (wTrim fields[0]!), mapping)

/-- Fresh parse of the pinned source, used only by the drift gate. -/
def widthCompatMappingsParsed : Array (Nat × Array Nat) :=
  ((unicodeDataRaw.splitOn "\n").filterMap wParseRow).toArray
