  ...
  if line.isEmpty then none
    ...
    | [rangeField, propField] =>
      if trimS propField = "Unified_Ideograph" then
        some (parseRange (trimS rangeField))
      else
        none
    | _ => none
...
/-- Inclusive ranges of codepoints with `Unified_Ideograph = Yes` in
    ...
    Table 16. -/
def unifiedIdeographRanges : Array (Nat × Nat) :=
...
theorem isUnifiedIdeograph_2B73A : isUnifiedIdeograph 0x2B73A = false := by native_decide

/-- 0x2B739 is the last assigned codepoint in CJK Ext C per
    UCD 16.0. -/
theorem isUnifiedIdeograph_2B739 : isUnifiedIdeograph 0x2B739 = true := by native_decide

/-- 0x4E00 (start of CJK Unified Ideographs) is Unified_Ideograph in
    every UCD version. -/
theorem isUnifiedIdeograph_4E00 : isUnifiedIdeograph 0x4E00 = true := by native_decide

/-- 0x2A6E0 is in the reserved gap between Ext B and Ext C — not
    Unified_Ideograph in any UCD version. -/
theorem isUnifiedIdeograph_2A6E0 : isUnifiedIdeograph 0x2A6E0 = false := by native_decide
