/-
  ...
  Literal Bidi_Mirroring_Glyph pairs from BidiMirroring.txt.
-/
...
def binarySearch (cp : Nat) (left right fuel : Nat) : Option Nat :=
  match fuel with
  | 0 => none
  | fuelNext + 1 =>
    if left < right then
      let mid := (left + right) / 2
      let pair := bidiMirrorPairs[mid]!
      if cp < pair.1 then binarySearch cp left mid fuelNext
      else if pair.1 < cp then binarySearch cp (mid + 1) right fuelNext
      else some pair.2
    else none
...
-- INTEGRITY GATE — `bidiMirrorPairs` must equal a fresh parse of BidiMirroring.txt.
...
def bmParse (line : String) : Option (Nat × Nat) :=
  let s := bStrip line
...
#eval do
  unless bidiMirrorPairs == bidiMirrorPairsParsed do
    throw (IO.userError "BidiMirroring drift: bidiMirrorPairs ≠ parsed BidiMirroring.txt")
