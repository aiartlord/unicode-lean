...
def render(report: dict[str, object]) -> str:
    ...
    if not corpus.get("available"):
        ...
        if not measured:
            ...
            add("")
            add("  The gap between those two rows is worth reading. Two of the")
            add("  trojan-source cases produce a hazard but not under")
            add("  source-display-divergence, which is the family they name. That")
            add("  detector's bidi constituent is BidiControlBalance, which fires on")
            add("  an unbalanced run, so a payload whose controls open and close")
            add("  leaves it clear: trojan-source-commenting-out carries an extra")
            add("  U+2066 and fires it, while stretched-string (U+2066/U+2069) and")
            add("  early-return (U+202D/U+202C) are balanced and do not. A working")
            add("  Trojan Source payload balances its controls so the file still")
            add("  parses, which is the case this constituent does not see. The")
            add("  attacks are still caught, by rtl-injection, filename-disguise and")
            add("  confusable-bidi-compound, so the shortfall is in attribution")
            add("  rather than detection.")
            add("")
            add("  domain-confusable-cyrillic-a is attribution too, by an explicit")
            add("  rule: the policy suppresses a homoglyph-confusable finding whose")
            add("  sub-threat is CrossScriptMix and reports the case under")
            add("  mixed-script-admissibility instead, so the family the corpus")
            add("  names cannot appear for a cross-script input. The fourth,")
            ...
            add("  carry the same weight as the attacks. The five that fire are not one")
            add("  kind of problem. korean-identifier-single-script is systematic:")
            add("  Unicode/Security/Boundary/IdentifierFormDrift.lean fires wherever a")
            add("  codepoint's identifier status differs from its NFKD head's, and every")
            add("  one of the 11,172 precomposed Hangul syllables is Allowed while its")
            add("  head jamo is not, so all Korean text fires it. That module's header")
            add("  anticipates this and directs callers to apply NFC first, but the")
            add("  precomposed syllables are already NFC, so the remedy it names does not")
            add("  change the input or the verdict. The other four are detectors reading")
            add("  arabic-string-literal-balanced is a second systematic case:")
            add("  RtlInjection.lean phase 1 is \"bidi format-control trumps all\", so")
            add("  the presence of any control fires it and balance is never")
            add("  consulted. bidi-control-balance, whose job is balance, correctly")
            add("  stays clear on this input. The finding is also tagged")
            add("  RloInLTRField, and confusable-bidi-compound tags")
            add("  ConfusableInOverride, though the codepoint here is U+202B")
            add("  RIGHT-TO-LEFT EMBEDDING and no override is present; both names")
            add("  describe an override the input does not contain. The remaining")
            add("  three are detectors reading a genuinely ambiguous input the way")
            add("  their contracts say to:")
            add("  Unicode/Security/Display/RtlInjection.lean carries")
            ...
            add("  control present, so a legitimate right-to-left comment is not")
            add("  distinguishable from a field takeover; the German eszett does")
            add("  expand under uppercasing and the Turkish dotted capital I does")
            add("  invert. Closing those is a change to the proven layer, not a")
