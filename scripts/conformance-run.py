...
def render(report: dict[str, object]) -> str:
    ...
    add("")
    add("  The vector files above are pinned, not executed. Their 657 rows across")
    add("  26 files are hash-verified by scripts/check-security-hashes.sh, and no")
    add("  harness reads them: each proves a hand-picked set written into the")
    add("  harness itself, either as a row list or as an inline conjunction. The")
    add("  two are not tied by a drift gate, unlike GraphemeBreakTest, whose")
    add("  materialized rowsList is mirrored against a fresh parse of its file. A")
    add("  reader should take the file as the provenance of the samples and the")
    add("  harness as the statement of what is proven, which is the smaller of the")
    add("  two.")
    add("")
