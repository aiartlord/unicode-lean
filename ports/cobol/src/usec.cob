...
DETECT-VARIATION.
    ...
    PERFORM VARYING IDX FROM 1 BY 1 UNTIL IDX > CP-COUNT
        IF (CP(IDX) >= 65024 AND CP(IDX) <= 65039) OR (CP(IDX) >= 917760 AND CP(IDX) <= 917999) OR (CP(IDX) >= 6155 AND CP(IDX) <= 6157)
            ADD 1 TO VS-COUNT
            IF NOT ((IDX > 1 AND CP(IDX - 1) = 169 AND CP(IDX) = 65039) OR (IDX > 1 AND CP(IDX - 1) = 48 AND CP(IDX) = 65024))
...
DETECT-MIXED-SCRIPT.
    ...
    PERFORM VARYING IDX FROM 1 BY 1 UNTIL IDX > CP-COUNT
        IF (CP(IDX) >= 65 AND CP(IDX) <= 90) OR (CP(IDX) >= 97 AND CP(IDX) <= 122)
            MOVE 1 TO HAS-LATN
        END-IF
        IF CP(IDX) >= 880 AND CP(IDX) <= 1023
            MOVE 1 TO HAS-GREK
        END-IF
        IF CP(IDX) >= 1024 AND CP(IDX) <= 1279
            MOVE 1 TO HAS-CYRL
        END-IF
    END-PERFORM
...
DETECT-RTL.
    ...
    PERFORM VARYING IDX FROM 1 BY 1 UNTIL IDX > CP-COUNT
        IF CP(IDX) = 8234 OR CP(IDX) = 8235 OR CP(IDX) = 8237 OR CP(IDX) = 8238 OR CP(IDX) = 8236 OR CP(IDX) = 8294 OR CP(IDX) = 8295 OR CP(IDX) = 8296 OR CP(IDX) = 8297
            ...
            MOVE CP-COUNT TO IDX
        ELSE
            IF (CP(IDX) >= 1424 AND CP(IDX) <= 1535) OR (CP(IDX) >= 1568 AND CP(IDX) <= 1791)
                IF FIRST-RTL = 0
...
DETECT-CONFUSABLE-BIDI.
    ...
    PERFORM VARYING IDX FROM 1 BY 1 UNTIL IDX > CP-COUNT
        IF CP(IDX) = 1072 OR CP(IDX) = 959 OR CP(IDX) = 1077 OR CP(IDX) = 946
...
DETECT-COVERT-DISPLAY.
    ...
    PERFORM VARYING IDX FROM 1 BY 1 UNTIL IDX > CP-COUNT
        ...
        IF CP(IDX) >= 917504 AND CP(IDX) <= 917631
            MOVE 1 TO HAS-TAG
        END-IF
        IF (CP(IDX) >= 65024 AND CP(IDX) <= 65039) AND NOT (IDX > 1 AND ((CP(IDX - 1) = 169 AND CP(IDX) = 65039) OR (CP(IDX - 1) = 48 AND CP(IDX) = 65024)))
...
SCAN-BIP39.
    MOVE 0 TO TRAILING-WS UPPER-FLAG DOUBLE-WS UNKNOWN-WORD PREV-WS
        ...
        ELSE
            ...
            ELSE
                IF UNKNOWN-WORD = 1
                    ...
                    PERFORM ADD-ALL-POS-FINDING
                ELSE
                    IF CP-COUNT = 4 AND CP(1) = 113 AND CP(2) = 122
...
ADD-ZERO-WIDTH-POS-FINDING.
    ...
    MOVE SPACES TO POS-TEXT
    PERFORM VARYING JDX FROM 1 BY 1 UNTIL JDX > CP-COUNT
        IF CP(JDX) = 8203 OR CP(JDX) = 8205 OR CP(JDX) = 8239 OR CP(JDX) = 8288 OR (CP(JDX) >= 65529 AND CP(JDX) <= 65531)
            COMPUTE POS-IDX = JDX - 1
            MOVE POS-IDX TO POS-NUM
