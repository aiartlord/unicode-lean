...
namespace afd = unicode_cpp::security::boundary::admissibility_form_drift;
...
std::filesystem::path data_dir() {
    ...
    throw std::runtime_error(
        "admissibility-form-drift test: cannot locate bundled UCD data");
}
...
afd::Verdict det(const std::vector<std::uint32_t>& in) {
    return afd::detect(tables(), as_span(in));
}
...
std::string read_fixture(std::string_view relative) {
    ...
    throw std::runtime_error("cannot locate admissibility-form-drift fixture");
}

}  // namespace

// Ground truth: every detect_* theorem in the verified Rust reference
// implementation. The shared context-free fixture carries the same vectors.

// ── §2 detect spot checks (one per Rust reference theorem) ───────────────

TEST_CASE("AdmissibilityFormDrift — detect_empty_clear") {
    // Both admissibility calls return false (empty is not an identifier), so
    // they agree.
    CHECK(det({}).classify.is_clear());
}

TEST_CASE("AdmissibilityFormDrift — detect_ascii_clear") {
    // "admin"; admissible on both sides (NFKC is identity).
    const auto v = det({0x61, 0x64, 0x6D, 0x69, 0x6E});
    CHECK(v.classify.is_clear());
    CHECK(v.input_admissible);
    CHECK(v.nfkc_admissible);
}

TEST_CASE("AdmissibilityFormDrift — detect_fi_ligature_drift") {
    ...
    const auto v = det({0xFB01});
    CHECK(v.classify.tag() == SV{"AdmissibilityFormDrift"});
    CHECK_FALSE(v.input_admissible);
    CHECK(v.nfkc_admissible);
}

TEST_CASE("AdmissibilityFormDrift — detect_jamo_sequence_drift") {
    // Decomposed Hangul jamos [U+1112, U+1161, U+11AB] are inadmissible, but
    // NFKC composes them to U+D55C 한 (admissible).
    using SV = std::optional<std::string_view>;
    CHECK(tag({0x1112, 0x1161, 0x11AB}) == SV{"AdmissibilityFormDrift"});
}

TEST_CASE("AdmissibilityFormDrift — reason_code_is_stable") {
    CHECK(policy::reason_code(Family::AdmissibilityFormDrift,
                              std::optional<std::string_view>{
                                  "AdmissibilityFormDrift"})
          == "unicode.security.X.admissibility-form-drift"
             ".AdmissibilityFormDrift");
}

// ── Shared context-free fixture through detect + reason-code wiring ──────

TEST_CASE("AdmissibilityFormDrift — shared detector fixture") {
    const auto text = read_fixture(
        "testdata/fixtures/security/detectors/admissibility_form_drift.json");
    const Json root = JsonParser{text}.parse();
    REQUIRE(root.field("schema").number == 1);
    CHECK(root.field("family").string == "admissibility-form-drift");
    ...
    for (const auto& test_case : root.field("cases").array) {
        ...
        // exactly as the sibling detectors do (Family::AdmissibilityFormDrift,
        // X layer).
