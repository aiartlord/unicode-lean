#include <doctest/doctest.h>

#include <cctype>
#include <cstddef>
#include <cstdint>
#include <filesystem>
#include <fstream>
#include <iterator>
#include <optional>
#include <span>
#include <stdexcept>
#include <string>
#include <string_view>
#include <utility>
#include <variant>
#include <vector>

#include "unicode_cpp/security/calculus.hpp"
#include "unicode_cpp/security/identity/emoji_zwj_integrity.hpp"
#include "unicode_cpp/security/policy.hpp"

namespace {

namespace ezwj = unicode_cpp::security::identity::emoji_zwj_integrity;
namespace policy = unicode_cpp::security::policy;
using unicode_cpp::security::Family;

std::filesystem::path data_dir() {
    const std::filesystem::path candidates[] = {"data", "../data", "../../data"};
    for (const auto& p : candidates) {
        if (std::filesystem::exists(p / "emoji-zwj-sequences.txt")) {
            return p;
        }
    }
    throw std::runtime_error(
        "emoji-zwj-integrity test: cannot locate bundled data");
}

const ezwj::RgiTable& rgi() {
    static const ezwj::RgiTable t = ezwj::RgiTable::load_from_dir(data_dir());
    return t;
}

std::span<const std::uint32_t> as_span(const std::vector<std::uint32_t>& v) {
    return std::span<const std::uint32_t>(v.data(), v.size());
}

ezwj::Verdict det(const std::vector<std::uint32_t>& in) {
    return ezwj::detect(rgi(), as_span(in));
}

std::optional<std::string_view> tag(const std::vector<std::uint32_t>& in) {
    return det(in).classify.tag();
}

// ── Minimal JSON reader for the shared detector fixture ─────────────────
struct Json {
    enum class Kind { Object, Array, String, Number };
    Kind kind = Kind::Number;
    std::vector<std::pair<std::string, Json>> object;
    std::vector<Json> array;
    std::string string;
    long number = 0;

    const Json& field(std::string_view key) const {
        for (const auto& [k, v] : object) {
            if (k == key) {
                return v;
            }
        }
        throw std::runtime_error("missing JSON field");
    }
};

class JsonParser {
public:
    explicit JsonParser(std::string_view input) : input_(input) {}

    Json parse() {
        skip_ws();
        Json value = parse_value();
        skip_ws();
        if (pos_ != input_.size()) {
            throw std::runtime_error("trailing JSON input");
        }
        return value;
    }

private:
    std::string_view input_;
    std::size_t pos_ = 0;

    void skip_ws() {
        while (pos_ < input_.size()
               && std::isspace(static_cast<unsigned char>(input_[pos_]))) {
            ++pos_;
        }
    }

    char peek() const {
        if (pos_ >= input_.size()) {
            throw std::runtime_error("unexpected end of JSON input");
        }
        return input_[pos_];
    }

    char take() {
        const char ch = peek();
        ++pos_;
        return ch;
    }

    void expect(char ch) {
        if (take() != ch) {
            throw std::runtime_error("unexpected JSON character");
        }
    }

    Json parse_value() {
        skip_ws();
        switch (peek()) {
        case '{':
            return parse_object();
        case '[':
            return parse_array();
        case '"':
            return parse_string_value();
        default:
            return parse_number();
        }
    }

    Json parse_object() {
        expect('{');
        skip_ws();
        Json out;
        out.kind = Json::Kind::Object;
        if (peek() == '}') {
            take();
            return out;
        }
        while (true) {
            skip_ws();
            std::string key = parse_string();
            skip_ws();
            expect(':');
            out.object.emplace_back(std::move(key), parse_value());
            skip_ws();
            const char ch = take();
            if (ch == '}') {
                break;
            }
            if (ch != ',') {
                throw std::runtime_error("expected object comma");
            }
        }
        return out;
    }

    Json parse_array() {
        expect('[');
        skip_ws();
        Json out;
        out.kind = Json::Kind::Array;
        if (peek() == ']') {
            take();
            return out;
        }
        while (true) {
            out.array.push_back(parse_value());
            skip_ws();
            const char ch = take();
            if (ch == ']') {
                break;
            }
            if (ch != ',') {
                throw std::runtime_error("expected array comma");
            }
        }
        return out;
    }

    Json parse_string_value() {
        Json out;
        out.kind = Json::Kind::String;
        out.string = parse_string();
        return out;
    }

    std::string parse_string() {
        expect('"');
        std::string out;
        while (true) {
            const char ch = take();
            if (ch == '"') {
                break;
            }
            if (ch != '\\') {
                out.push_back(ch);
                continue;
            }
            const char escaped = take();
            switch (escaped) {
            case '"':
            case '\\':
            case '/':
                out.push_back(escaped);
                break;
            case 'n':
                out.push_back('\n');
                break;
            case 'r':
                out.push_back('\r');
                break;
            case 't':
                out.push_back('\t');
                break;
            default:
                throw std::runtime_error("unsupported string escape");
            }
        }
        return out;
    }

    Json parse_number() {
        bool negative = false;
        if (peek() == '-') {
            negative = true;
            take();
        }
        long value = 0;
        bool saw_digit = false;
        while (pos_ < input_.size()
               && std::isdigit(static_cast<unsigned char>(input_[pos_]))) {
            saw_digit = true;
            value = value * 10 + (take() - '0');
        }
        if (!saw_digit) {
            throw std::runtime_error("expected JSON number");
        }
        Json out;
        out.kind = Json::Kind::Number;
        out.number = negative ? -value : value;
        return out;
    }
};

std::string read_fixture(std::string_view relative) {
    const std::filesystem::path rel{relative};
    const std::filesystem::path candidates[] = {
        rel, std::filesystem::path{".."} / rel,
        std::filesystem::path{"../.."} / rel};
    for (const auto& path : candidates) {
        if (!std::filesystem::exists(path)) {
            continue;
        }
        std::ifstream in(path, std::ios::binary);
        return std::string(std::istreambuf_iterator<char>{in},
                           std::istreambuf_iterator<char>{});
    }
    throw std::runtime_error("cannot locate emoji-zwj-integrity fixture");
}

}  // namespace

// ── §3/§4 data-layer sanity ─────────────────────────────────────────────

TEST_CASE("EmojiZwjIntegrity — is_emoji_modifier range") {
    CHECK(ezwj::is_emoji_modifier(0x1F3FB));
    CHECK(ezwj::is_emoji_modifier(0x1F3FF));
    CHECK(!ezwj::is_emoji_modifier(0x1F3FA));
    CHECK(!ezwj::is_emoji_modifier(0x1F600));
}

TEST_CASE("EmojiZwjIntegrity — ZWJ alphabet admits heart/man, rejects grinning") {
    // U+2764 HEAVY BLACK HEART appears in couple-with-heart RGI sequences.
    CHECK(rgi().is_emoji_target(0x2764));
    // U+1F468 MAN appears in family/couple RGI sequences.
    CHECK(rgi().is_emoji_target(0x1F468));
    // U+1F600 GRINNING FACE appears in no registered RGI ZWJ sequence.
    CHECK(!rgi().is_emoji_target(0x1F600));
    // The joiner itself is excluded from the alphabet.
    CHECK(!rgi().is_emoji_target(ezwj::ZWJ));
}

TEST_CASE("EmojiZwjIntegrity — registered membership is exact") {
    // MAN + ZWJ + LAPTOP (man technologist) is a registered RGI sequence.
    const std::vector<std::uint32_t> man_laptop = {0x1F468, 0x200D, 0x1F4BB};
    CHECK(rgi().is_registered_zwj_sequence(as_span(man_laptop)));
    // MAN + ZWJ + WOMAN is not a registered RGI sequence.
    const std::vector<std::uint32_t> man_woman = {0x1F468, 0x200D, 0x1F469};
    CHECK(!rgi().is_registered_zwj_sequence(as_span(man_woman)));
}

// ── §5 detect spot checks (one per Rust reference theorem) ───────────────

TEST_CASE("EmojiZwjIntegrity — detect_empty_clear") {
    const auto v = det({});
    CHECK(v.classify.is_clear());
    CHECK(v.classify.tag() == std::nullopt);
    CHECK(v.zwj_positions == std::vector<std::size_t>{});
    CHECK(v.chain_length == 0);
    CHECK(v.skin_tone_count == 0);
}

TEST_CASE("EmojiZwjIntegrity — detect_ascii_clear") {
    CHECK(det({0x48, 0x65, 0x6C, 0x6C, 0x6F}).classify.is_clear());
}

TEST_CASE("EmojiZwjIntegrity — detect_plain_emoji_clear") {
    CHECK(det({0x1F600}).classify.is_clear());
}

TEST_CASE("EmojiZwjIntegrity — detect_one_skintone_clear") {
    const auto v = det({0x1F44B, 0x1F3FB});
    CHECK(v.classify.is_clear());
    CHECK(v.skin_tone_count == 1);
}

TEST_CASE("EmojiZwjIntegrity — detect_family_rgi_clear") {
    const auto v =
        det({0x1F468, 0x200D, 0x1F469, 0x200D, 0x1F467, 0x200D, 0x1F466});
    CHECK(v.classify.is_clear());
    CHECK(v.is_registered_rgi);
}

TEST_CASE("EmojiZwjIntegrity — detect_double_zwj") {
    using SV = std::optional<std::string_view>;
    const auto v = det({0x1F600, 0x200D, 0x200D, 0x1F600});
    CHECK(v.classify.tag() == SV{"DoubleZWJ"});
    CHECK(v.classify.positions == std::vector<std::size_t>{1});
}

TEST_CASE("EmojiZwjIntegrity — detect_non_emoji_injection") {
    using SV = std::optional<std::string_view>;
    const auto v = det({0x1F600, 0x200D, 0x0061});
    CHECK(v.classify.tag() == SV{"NonEmojiInjection"});
}

TEST_CASE("EmojiZwjIntegrity — detect_skin_tone_overflow") {
    using SV = std::optional<std::string_view>;
    const auto v =
        det({0x1F44B, 0x1F3FB, 0x1F3FC, 0x1F3FD, 0x1F3FE, 0x1F3FF});
    CHECK(v.classify.tag() == SV{"SkinToneOverflow"});
    CHECK(v.skin_tone_count == 5);
}

TEST_CASE("EmojiZwjIntegrity — detect_man_laptop_registered_clear") {
    CHECK(det({0x1F468, 0x200D, 0x1F4BB}).classify.is_clear());
}

TEST_CASE("EmojiZwjIntegrity — detect_unregistered") {
    using SV = std::optional<std::string_view>;
    // man + ZWJ + woman: both flanks are in the RGI alphabet but the joined
    // sequence is not registered.
    const auto v = det({0x1F468, 0x200D, 0x1F469});
    CHECK(v.classify.tag() == SV{"UnregisteredSequence"});
}

TEST_CASE("EmojiZwjIntegrity — detect_grinning_laptop_non_emoji_injection") {
    using SV = std::optional<std::string_view>;
    // grinning face is not a valid ZWJ-join target, so this surfaces as
    // NonEmojiInjection.
    CHECK(tag({0x1F600, 0x200D, 0x1F4BB}) == SV{"NonEmojiInjection"});
}

// ── structural checks (follow from the priority ladder) ──────────────────

TEST_CASE("EmojiZwjIntegrity — over_length_fires_past_cap") {
    using SV = std::optional<std::string_view>;
    // 9 men joined by 8 ZWJs = 17 codepoints (> MAX_RGI_LENGTH).
    std::vector<std::uint32_t> input;
    for (int i = 0; i < 9; ++i) {
        if (i > 0) {
            input.push_back(0x200D);
        }
        input.push_back(0x1F468);
    }
    CHECK(input.size() == 17);
    const auto v = det(input);
    CHECK(v.classify.tag() == SV{"OverLength"});
    REQUIRE(v.classify.sub.has_value());
    const auto* over = std::get_if<ezwj::OverLength>(&*v.classify.sub);
    REQUIRE(over != nullptr);
    CHECK(over->length == 17);
    CHECK(over->max_length == ezwj::MAX_RGI_LENGTH);
    CHECK(v.classify.positions == std::vector<std::size_t>{});
}

TEST_CASE("EmojiZwjIntegrity — trailing_zwj_is_injection") {
    using SV = std::optional<std::string_view>;
    const auto v = det({0x1F468, 0x200D});
    CHECK(v.classify.tag() == SV{"NonEmojiInjection"});
    CHECK(v.classify.positions == std::vector<std::size_t>{1});
    REQUIRE(v.classify.sub.has_value());
    const auto* inj = std::get_if<ezwj::NonEmojiInjection>(&*v.classify.sub);
    REQUIRE(inj != nullptr);
    CHECK(inj->zwj_pos == 1);
    CHECK(inj->non_emoji_cp == 0);
}

TEST_CASE("EmojiZwjIntegrity — double_zwj_beats_unregistered") {
    using SV = std::optional<std::string_view>;
    // man ZWJ ZWJ boy — adjacent ZWJs present.
    const auto v = det({0x1F468, 0x200D, 0x200D, 0x1F466});
    CHECK(v.classify.tag() == SV{"DoubleZWJ"});
}

// ── Shared context-free fixture through detect + reason-code wiring ──────

TEST_CASE("EmojiZwjIntegrity — shared detector fixture") {
    const auto text = read_fixture(
        "testdata/fixtures/security/detectors/emoji_zwj_integrity.json");
    const Json root = JsonParser{text}.parse();
    REQUIRE(root.field("schema").number == 1);
    CHECK(root.field("family").string == "emoji-zwj-integrity");

    for (const auto& test_case : root.field("cases").array) {
        const std::string name = test_case.field("name").string;
        INFO(name);

        std::vector<std::uint32_t> input;
        for (const auto& value : test_case.field("input").array) {
            input.push_back(static_cast<std::uint32_t>(value.number));
        }

        const auto verdict = det(input);

        // Reason code the policy layer would emit for this verdict, wired
        // exactly as the sibling identity detectors do
        // (Family::EmojiZwjIntegrity, I layer).
        std::optional<std::string> emitted;
        if (auto verdict_tag = verdict.classify.tag()) {
            emitted = policy::reason_code(
                Family::EmojiZwjIntegrity,
                std::optional<std::string_view>{*verdict_tag});
        }

        const auto& required = test_case.field("required_findings").array;
        if (required.empty()) {
            CHECK(verdict.classify.is_clear());
        } else {
            REQUIRE(emitted.has_value());
            for (const auto& finding : required) {
                CHECK(*emitted == finding.string);
            }
        }
    }
}
